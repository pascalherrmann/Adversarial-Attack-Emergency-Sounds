...
class RobustnessExperiment():
    ...
    def run(self, model_path, module_class):
        ...
        for a in range(len(self.experiment_configs)):
            ...
            title = current_attack.__name__  if not exp_conf["meta"]["title"] else exp_conf["meta"]["title"]
            ...
            for c in tqdm(range(len(configs))):
                result = model.report(attack = current_attack, attack_args = configs[c], log=False)
                current_attack_report.append(result)
            
            key_x = exp_conf["meta"]["key_config"]
            key_y = exp_conf["meta"]["key_result"]
            xs = [ res[key_x] for res in configs ] 
            ys = [ res[key_y] for res in current_attack_report ]

            vis_object = [{"data": ys, "color" : "rbgycmk"[0], "label": key_y}]

            drawPlot(x = xs, data = vis_object, x_label = key_x, y_label = key_y, title = title + " | model:" + model_name, save_path = os.path.join(results_dir, current_attack.__name__ + ".pdf"))
                
            save_json(current_attack_report, os.path.join(results_dir, current_attack.__name__ + ".json"))
            
            info = "Attack: \t" + current_attack.__name__ + "\n"
            info += key_x + "\t\t" + str(xs) + "\n"
            info += key_y + "\t\t" + str(ys) + "\n\n"
            write_to_file(info, os.path.join(self.dir, "summary.txt"))
